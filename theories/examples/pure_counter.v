(** * A pure Counter in slot form: callbacks are abstract transitions,
...
>>
    ...
    - [click_spec]: the handler takes [slot_ptsto γ (cint n)] to
      [slot_ptsto γ (cint (n+2))] — the specification of the callback as a
      transition of the abstract state, with the physical queueing of
      the two updaters as a side product;
    - [body_succ_spec]: on re-render, from [slot_ptsto γ a] the body renders
    ...
    slot layer: its queue is not pure, so [slot_res] cannot be
...
Section pure_counter.
  ...
  Lemma click_spec (γ : gname) (n ns nx : Z) (p : path) (π : domains.view)
    ...
    slot_ptsto γ (cint n) -∗ slot_res δ is_int γ ent -∗
    mem_auth_frag m -∗ view_ptsto p π -∗ reg_token None -∗
    (∀ π' ent', slot_ptsto γ (cint (n + 1 + 1)) -∗ slot_res δ is_int γ ent' -∗
  ...
  Proof.
    ...
    do 5 wp_pure.
    iApply (wp_setter_normal_slot _ is_int γ (cint n) finc with "[] Hm Hs Hmem Hv Hr");
      [done|done|apply finc_int|iApply upd_pure_inc|].
    iNext. iIntros "Hm Hs Hmem Hv Hr".
    do 5 wp_pure.
    iApply (wp_setter_normal_slot _ is_int γ _ finc with "[] Hm Hs Hmem Hv Hr");
  ...
  Lemma body_succ_spec (γ : gname) (a : domains.val) (nx : Z) (p : path)
      ...
      (Φ : mval → iProp Σ) :
    vw_sttst π !! 0 = Some ent →
    slot_ptsto γ a -∗ slot_res δ is_int γ ent -∗ render_ctx p π -∗
    (∀ n, ⌜a = cint n⌝ -∗
       slot_ptsto γ a -∗ slot_res δ is_int γ (StEntry a []) -∗
